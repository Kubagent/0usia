...
export default function SimpleHero() {
  ...
  return (
    <>
      ...
      {/* <div className="fixed bottom-4 left-4 z-50 bg-black bg-opacity-80 text-white p-3 text-xs rounded font-mono">
        <div>Manual Progress: {(scrollProgress * 100).toFixed(1)}%</div>
        <div>BG: {fallbackBgColor}</div>
        <div>Filter: {fallbackLogoFilter}</div>
        <div>Framer Y: {scrollYProgress.get().toFixed(3)}</div>
      </div> */}
